...
def genetic_algorithm(bounds, population_size, num_generations, mutation_rate):
    ...
    for i in range(num_generations):
        ...
        # offspring = crossover(parents, population_size, number2mate, bounds)
        ...
        # TRACK BEST MASS FOR EACH GENERATION
        mass_track[i, 0] = np.min(DV_chars[:, 1])
        mass_track[i, 1] = gen
        gen += 1
        del DV_chars
    ...
    return best_design, best_mass, mass_track


# OPTIMIZATION START
# DENOTE DV BOUNDS
dim_bounds = np.array([[0.05, 0.05, 3.00, 0.50, 0.50],
                      [1.00, 1.00, 30.0, 2.00, 2.00]])
#                     [[t_skinL, t_stiffL, n_stiffL, h_stiffL, w_stiffL],
#                     [t_skinU, t_stiffU, n_stiffU, h_stiffU, w_stiffU]]

# RUN OPTIMIZER
opt_soln, mass, mass_gen = genetic_algorithm(dim_bounds, population_size=100, num_generations=100, mutation_rate=0.05)
round_num = 4
